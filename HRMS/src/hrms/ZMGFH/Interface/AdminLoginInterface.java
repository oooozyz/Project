...
public class AdminLoginInterface extends JDialog {
	...
	public AdminLoginInterface(Frame f) {
		super(f, "管理员登录", true);
	...
	private void addListener() {
		...
		cancelButton.addActionListener(new ActionListener() {// 取消按钮的事件监听
			...
			public void actionPerformed(ActionEvent e) {
				AdminLoginInterface.this.dispose();// 销毁登录对话框
			}
		...
		loginButton.addActionListener(new ActionListener() {// 确定按钮的事件监听
			...
			public void actionPerformed(ActionEvent e) {
				...
				boolean judge = HRService.adminLogin(username, password);
				if (judge) {
					AdminLoginInterface.this.dispose();// 销毁登录对话框
				} else {
					JOptionPane.showMessageDialog(AdminLoginInterface.this, "用户名或密码有误！");
				}
